...
sequenceDiagram
    ...
    participant V as 虚拟主播
    participant T as TRTC 云端
    participant U as 观众端

    Note over A,S: 创建虚拟房间

    A->>S: StartAccompany(比赛ID)
    S-->>A: 返回虚拟房间信息

    Note over A,V: 进入子房间并订阅比赛流

    A->>T: SubRoomService 创建并进入 TRTC 子房间
    V->>T: 推送比赛视频流到子房间
    A->>T: startRemoteView 订阅比赛流
    T-->>A: 比赛流（本地预览）

    Note over A,T: 本地推流与云端混流

    A->>T: OBS 推送本地画面（摄像头/图片）
    T->>T: 云端混流（比赛背景 + 主播画中画）
    T-->>U: 分发混合流

    Note over A,U: 状态同步（SEI）
...
<p style="text-align:center;color:#94a3b8;font-size:13px;margin-top:-6px;">图 5｜陪看核心流程（时序）</p>
...
sequenceDiagram
    autonumber
    participant U as 主播端用户
    participant R as Renderer 进程
    participant M as Main 进程
    participant TC as TRTCController
    participant T as TRTC 云端
    participant V as 观众端

    Note over U,R: 状态变更触发

    U->>R: 触发状态变更（静音 / 音量 / 连麦态）
    R->>R: 构建 SEI 数据

    Note over R,TC: 跨进程下发到 SDK

    R->>M: IPC 发送 SEI 数据
    M->>TC: TRTCController.sendSEIMsg

    Note over TC,V: 视频流附带 SEI 并分发
...
<p style="text-align:center;color:#94a3b8;font-size:13px;margin-top:-6px;">图 6｜SEI 状态同步链路</p>
