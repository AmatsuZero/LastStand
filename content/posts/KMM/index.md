...
**KMM** 全称 **Kotlin Multiplatform Mobile**，是 JetBrains 推出的代码复用方案，基于 Kotlin 语言，主要实现 iOS、Android 两端业务 / 逻辑代码共享，平台独有能力由两端原生实现。
和 RN/Flutter 区别：RN/Flutter 是全栈跨端（UI + 逻辑），拥有独立渲染引擎、JS/Dart 运行时，存在跨语言通信、性能损耗、原生能力兼容问题；KMM只共享业务逻辑，UI 完全使用原生，保留原生体验、原生手势、原生性能，无独立渲染层。
和纯原生混编 SDK 区别：传统混编是单独编译静态库 / Framework，代码无法双向复用；KMM 是一套源码多端编译，基于 Kotlin 多平台源码集做分层，天然支持平台差异化逻辑。
...
原版 Compose Multiplatform 在 iOS 平台上的混排限制根植于其 Skia 渲染引擎与 Apple 原生 UIKit/SwiftUI 框架之间的架构性差异，具体表现为渲染层级冲突、内存性能压力以及交互体验割裂三大核心问题。ovCompose 通过一系列创新性的架构改进，系统性地解决了这些限制：双渲染模式架构打破了单一渲染后端的束缚，使开发者能够按场景灵活选择 Skia 或 UIKit 渲染策略；PictureRecorder 局部更新和 UIKit 原生容器优化显著降低了内存占用和计算开销；ComposeUIView 包装器、Intrinsic Content Size 支持和协作式触摸处理则从工程实践层面提升了混排的集成效率和交互体验。这些技术改进使 ovCompose 成为 Compose Multiplatform 生态中解决 iOS 混排难题的重要实践方案，为跨平台 UI 开发在 iOS 端的深度落地提供了可行的技术路径。
...
ovCompose（Online Video Compose）是由腾讯视频 Oteam 前端团队开发并开源的跨平台 UI 框架，深度集成于 JetBrains 的 Compose Multiplatform (CMP) 生态系统 [1](https://github.com/Tencent-TDS/ovCompose-multiplatform-core)。该项目的核心使命是解决标准 CMP 在实际生产环境中的三大痛点：iOS 平台原生混排能力薄弱、内存占用过高，以及官方版本对鸿蒙（HarmonyOS）系统的支持缺失 [2]。

ovCompose 实现了 Android、iOS 和 HarmonyOS 的"三端一码"架构，代码复用率高达 90% [3]。在 iOS 端，ovCompose 引入了双渲染模式（Skia/UIKit 运行时切换）、PictureRecorder 增量更新架构以及 ComposeUIView 包装器等创新机制，有效解决了混排渲染层级冲突、手势竞争和内存压力等问题 [1](https://github.com/Tencent-TDS/ovCompose-multiplatform-core)。在鸿蒙端，ovCompose 则承担了更为根本性的使命——从零构建 Compose Multiplatform 在鸿蒙平台上的完整运行环境。

#### 为何需要适配鸿蒙

JetBrains 官方 Compose Multiplatform 截至 2026 年仍未正式支持 HarmonyOS NEXT 的原生 UI 渲染 [4]。这一缺失源于两个层面的技术壁垒：

首先，HarmonyOS NEXT 采用了全新的 ArkUI 声明式 UI 框架和自研的方舟编译器（Ark Compiler），其底层渲染管线与 Android 的 Skia 图形栈和 iOS 的 Core Animation/Metal 管线存在本质差异。官方 CMP 的渲染后端（Skiko）需要针对鸿蒙的 Native Window 系统和图形驱动接口进行深度适配，而非简单的平台抽象层封装。

其次，HarmonyOS NEXT 的应用开发语言以 ArkTS 为主，其运行时环境与 Kotlin/Native 编译产物的交互需要通过 NAPI（Native API）桥接层实现。这意味着 Compose 的 Kotlin 代码无法直接运行在鸿蒙的 JS/ArkTS 运行时之上，必须编译为原生机器码并通过 C/C++ 接口与系统交互 [4]。

面对这一技术空白，腾讯视频团队选择自研 ovCompose 框架来填补鸿蒙平台的 Compose 生态缺口，使腾讯视频成为鸿蒙平台上首个采用全跨端架构落地的大型应用 [3]。
...
ovCompose 在鸿蒙平台上的适配代表了 Compose Multiplatform 生态向新操作系统延伸的一次重要技术探索。其核心贡献可以概括为三个层面：

在**编译层面**，ovCompose 验证了 Kotlin/Native 直接适配鸿蒙 LLVM 工具链的可行性，为后续 Kotlin 生态项目提供了可复用的技术路径。在**渲染层面**，ovCompose 完成了 Skia 引擎与鸿蒙 Native Window 系统的对接，并通过三明治架构解决了跨平台框架中普遍存在的原生组件混排难题。在**工程层面**，ovCompose 以腾讯视频这一大体量应用为验证载体，证明了全跨端架构在鸿蒙平台上的生产可用性。

随着 HarmonyOS NEXT 生态的持续扩展和 JetBrains 官方对鸿蒙支持的潜在推进，ovCompose 的开源将为更广泛的开发者社区提供宝贵的参考经验和技术基础。
