...
| 阶段 | 引擎侧带过来的能力 |
|---|---|
| Swift 友好标注阶段 | Swift 友好语义标注（`NS_SWIFT_SENDABLE` / `NS_SWIFT_UI_ACTOR` / `NS_ENUM`） |
| OpenAL 正式修复阶段 | OpenAL 退出期 `s_isEngineDestroying` atomic 守卫 |
| OpenAL 临时止血阶段 | OpenAL 退出期临时止血版本（注释掉 `alSourceStop`） |
| 业务侧补丁阶段 | setting path 路径修复 + DataManager 清理逻辑等业务侧零散更新 |
| 子模块滚动更新阶段 | 子模块滚动更新（与 Swift 友好标注阶段一同支撑上线） |

每次更新子模块还顺带更新了 `MagicDanmakuiOS/Headers/*.h`（这层做的是 Headers 镜像）和
`MagicDanmakuiOS/libs/libcocos_engine.a`（预编译的静态库，避免业务方再编一遍引擎）。
比如 Swift 友好标注阶段同时改了 5 个头文件 + 重新打包了 `libcocos_engine.a` 和
`libboost_container.a`。

### 5.1 MagicDanmaku 拆包与分阶段加载

除了上面的引擎滚更，`MagicDanmaku` 业务层在这一段还做了一次关键的加载链路优化：

- **阶段一（资源拆分）**：把网球资源从单一大包拆成 4 个子 Bundle（player / animation / audio / audience）；
- **阶段二（路径收敛）**：把子包提升为顶级 bundle 路径，Native 侧直接按本地目录加载；
- **阶段三（链路补齐）**：补齐音频拆包后的动态绑定链路（绑定后再 `setup`）。
...
| 优化项 | 类别 | 改动量 | 主要收益 |
|---|---|---|---|
| 拆包 + 分阶段加载 | 资产 + 代码 | 3 个迭代阶段（资源拆分→路径收敛→链路补齐） | 首屏加载：1.5s → 500ms（约 +67%） |
