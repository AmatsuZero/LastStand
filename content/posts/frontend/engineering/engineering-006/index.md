...
具体的配置教程可以参考[官方配置文档](https://link.juejin.cn/?target=https%3A%2F%2Feslint.org%2Fdocs%2Flatest%2Fuser-guide%2Fconfiguring%2F)，不是这里三两句能说完的。

在这里，我使用的是 [airbnb-base](https://link.juejin.cn/?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Feslint-config-airbnb-base) 规范。
...
以 VSCode 为例，在 VSCode 上使用 ESLint 需要安装 [ESLint 插件](https://link.juejin.cn/?target=https%3A%2F%2Fmarketplace.visualstudio.com%2Fitems%3FitemName%3Ddbaeumer.vscode-eslint)。启用插件后，可以在编辑代码的同时看到哪些代码有问题，及时发现，及时修复。使用鼠标 hover 红线，或者在下方的 PROBLEMS 面板中都可以看到具体的错误提示。

![](image-02.webp) Awesome！不过，问题来了：
...
> 💡 如果插件无法正确读取项目中的 ESLint 程序和配置，会导致插件启动失败。这个问题常常是因为 ESLint 没有安装在打开目录的根部。可以通过修改[插件的配置](https://link.juejin.cn/?target=https%3A%2F%2Fmarketplace.visualstudio.com%2Fitems%3FitemName%3Ddbaeumer.vscode-eslint%23settings-options)解决。
...
WebStorm 用户可以参考[这里](https://link.juejin.cn/?target=https%3A%2F%2Fwww.jetbrains.com%2Fhelp%2Fwebstorm%2Feslint.html%23ws_eslint_configure_run_eslint_on_save)设置。

启用后，就几乎不用手动执行 `eslint` 命令了。

## 2. Prettier

另一个广为人知的工具是 [Prettier](https://link.juejin.cn/?target=https%3A%2F%2Fprettier.io%2F)，它是一个代码格式化工具。
...
要同时使用二者，就需要关闭 ESLint 中可能和 Prettier 冲突的规则，而这就是 [eslint-config-prettier](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fprettier%2Feslint-config-prettier) 所做的事。所以，想同时使用两者，你需要在 ESLint 中使用该配置，具体的配置方式参考使用文档即可。

如果你只想在 JavaScript 中使用 ESLint，可以在 `.prettierignore` 中忽略所以的 JavaScript 文件即可。

## 3. Stylelint

顾名思义，[Stylelint](https://link.juejin.cn/?target=https%3A%2F%2Fstylelint.io%2F) 就是样式代码的 linter。

> A mighty CSS linter that helps you avoid errors and enforce conventions.

它的功能和使用，都和 ESLint 类似，不过作用目标不同而已。

和 Prettier 的区别在于，它和 ESLint 一样，是一个 linter，会进行语义分析，能发现一些模式问题。在 Stylelint 15 之前，如果同时使用 Stylelint 和 Prettier，也需要使用 [stylelint-config-prettier](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fprettier%2Fstylelint-config-prettier) 避免在样式文件上的规则冲突。Stylelint 15 废弃了所有风格规则，不会和 Prettier 冲突。
...
默认的 Git hook 不容易设置，社区中流行使用 [husky](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Ftypicode%2Fhusky) 进行配置。

每次提交时的检查应该是针对当前 commit 内修改的内容，而不是全部文件，也就是只检查暂存区内的文件。[🚫💩lint-staged](https://link.juejin.cn/?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Flint-staged)可以实现这种效果。
...
[commitlint - Lint commit messages](https://link.juejin.cn/?target=https%3A%2F%2Fcommitlint.js.org%2F%23%2F) 是规范提交消息（commit message）的一个工具，可以避免有些小伙伴就喜欢 `git commit -m "u"`，要求一个提交消息符合[Conventional Commits](https://link.juejin.cn/?target=https%3A%2F%2Fwww.conventionalcommits.org%2Fen%2Fv1.0.0%2F)规范，即以下形式：
...
如果希望规范团队项目中的分支名称，可以使用[branch-name-lint](https://link.juejin.cn/?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Fbranch-name-lint)。
...
[EditorConfig](https://link.juejin.cn/?target=https%3A%2F%2Feditorconfig.org%2F)是一个跨编辑器/IDE 的编辑器配置工具。与 linter 和 formatter 不同，它的作用对象不是代码，而是去约束编辑器的配置，比如缩进风格是 tab 还是空格，缩进多少个字符。同时也说明，它是作用于文件级别的，对代码语法是无感的，对所有文件都生效。
