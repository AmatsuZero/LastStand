...
- 请列出一些常见触发 BFC 的方式，并解释每种方式的用法。<br>
  - `float` 不为 `none`。
...
- BFC 有哪些重要的布局特性？如何利用这些特性解决实际布局问题？<br>
...
- **清除浮动**：如何使用 BFC 来清除浮动？为什么比使用 `clearfix` 更简便？<br>
  - 示例：父元素 `overflow: hidden` 即可清除子元素的浮动。
- **防止外边距重叠**：如何利用 BFC 解决外边距折叠问题？<br>
...
- 示例问题：如何使用 BFC 实现一个固定宽度的侧边栏和一个自适应的内容区？<br>
```html
<div class="container">
