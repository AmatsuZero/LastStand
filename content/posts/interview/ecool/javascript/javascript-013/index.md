...
draft = true
weight = 13
tags = ["面试", "前端", "JavaScript", "异步编程", "ecool"]
categories = ["前端开发", "面试"]
...
本文将按照异步编程方式的出现时间来归纳整理： ![](image-01.webp)
...
![](image-02.webp) 可以看到，async 函数返回的是 Promise 对象。如果异步函数使用return关键字返回了值（如果没有return则会返回undefined），这个值则会被 `Promise.resolve()` 包装成 Promise 对象。异步函数始终返回Promise对象。
