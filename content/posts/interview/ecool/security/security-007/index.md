...
-  **前端校验**：<br>
  - 使用 `accept` 属性限制文件类型：<br>
```html
<input type="file" accept=".jpg, .png, .pdf" />
```
  - 通过 JavaScript 检查 MIME 类型或扩展名。
-  **后端校验**：<br>
  - 解析文件头（Magic Number），验证文件的真实类型。<br>
```python
...
```
- **服务器配置**：<br>
  - Nginx 或 Apache 等 Web 服务器可配置最大请求体大小。<br>
```
...
-  **存储位置**：<br>
  - **隔离存储**：将文件存储在独立的非 Web 根目录，防止通过 URL 直接访问文件。<br>
```
/uploads/
/var/www/myapp/
```
...
-  **病毒扫描**：<br>
  - 使用杀毒软件或工具（如 ClamAV）对上传的文件进行扫描。<br>
```bash
