...
| 文章 | 定位 |
|-----|------|
| [APM](APM.md)（本文） | 系列入口：APM/RUM 定义、B/C 端边界、技术分层、建设路线 |
| [APM-产品与架构设计](APM-产品与架构设计.md) | 产品视角：C 端体验、B 端用户、角色视图、治理闭环 |
| [APM-指标体系](APM-指标体系.md) | 指标口径：稳定性、流畅性、启动、资源、网络、业务、RUM 指标 |
| [APM-C端SDK架构](APM-C端SDK架构.md) | iOS SDK 架构：插件化、远程配置、采样、隐私、低开销、防自崩 |
| [APM-数据采集](APM-数据采集.md) | 采集技术：Crash、Watchdog、FOOM、卡顿、启动、网络、MetricKit、业务 Trace |
| [APM-数据模型与上报](APM-数据模型与上报.md) | 事件模型、RUM ID、协议、落盘、批量、重试、脱敏、采样 |
| [APM-服务端数据架构](APM-服务端数据架构.md) | 接入网关、消息队列、实时计算、存储、符号化、Issue 聚合、配置下发 |
| [APM-B端平台设计](APM-B端平台设计.md) | Web 控制台：大盘、详情页、用户会话、告警、工单、发布防劣化 |
| [APM-业界方案](APM-业界方案.md) | MetricKit、Sentry、Firebase、Bugly、Matrix、Slardar、Hertz 等方案对比 |
...
- 崩溃体系：[崩溃](../崩溃/崩溃.md)、[崩溃-采集](../崩溃/崩溃-采集.md)、[崩溃-治理](../崩溃/崩溃-治理.md)
- 卡顿体系：[卡顿](../卡顿/卡顿.md)、[卡顿-检测](../卡顿/卡顿-检测.md)
- 启动体系：[启动优化](../启动优化/启动优化.md)、[启动优化-观测](../启动优化/启动优化-观测.md)
- 耗电体系：[耗电](../耗电/耗电.md)、[耗电-检测](../耗电/耗电-检测.md)、[耗电-治理](../耗电/耗电-治理.md)
- 工具体系：[Instruments详解](../Instruments详解.md)
