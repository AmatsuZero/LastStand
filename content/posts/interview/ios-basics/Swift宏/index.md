...
| KeyPath + Mirror | 运行时反射读写属性 | 只能读，类型安全差，性能差（见 [iOS反射]({{< relref "/posts/interview/ios-basics/iOS反射" >}})） |
...
iOS 26 端侧 LLM 框架（见 [FoundationModels]({{< relref "/posts/interview/ai/FoundationModels" >}})）用 `@Generable` 把 Swift 类型变成模型可填的 Schema：
