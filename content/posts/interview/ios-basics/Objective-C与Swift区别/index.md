...
- 动态性由 Objective-C Runtime 系统支持，可以在运行时修改类结构、方法交换等，详见 [Runtime](./runtime.md)
...
- 引用其他文件/模块必须要`import`（[Objective-C中import详解](./Objective-C中import详解.md)）
- 协议不支持默认实现
...
- 不需要单独的头文件声明接口
- 默认情况下变量必须初始化
- 引用同模块的其他文件不需要`import`（[Swift中import详解](./Swift中import详解.md)）
...
拓展：[值类型和引用类型的区别](./值类型和引用类型的区别.md)
拓展：[iOS中的内存管理](./iOS中的内存管理.md)
...
拓展：[OOP、POP与AOP](../ios-advanced/OOP-POP-AOP.md)

## 方法派发方式

### Objective-C
- **方法调用**只支持消息派发（动态派发）
- 方法调用本质是消息发送（`objc_msgSend`），详见 [Runtime](./runtime.md)
...
拓展：[Swift底层原理-结构体、类和协议](./Swift底层原理-结构体、类和协议.md)
...
- 预处理阶段处理`#import`和宏展开（详见 [Objective-C中import详解](./Objective-C中import详解.md)）
...
拓展：[Mach-O的链接、装载与库](./Mach-O的链接、装载与库.md)
...
拓展：[Objective-C底层原理-NSObject](./Objective-C底层原理-NSObject.md)
拓展：[Swift底层原理-结构体、类和协议](./Swift底层原理-结构体、类和协议.md)
