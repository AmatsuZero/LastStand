...
在前一篇文章[Objective-C中import详解](./Objective-C中import详解.md)中，我们深入探讨了 Objective-C 中的 import 机制，包括 `#include`、`#import`、`@import` 以及 Clang Modules 的工作原理。本文将继续从 Swift 编译器的角度，讲解 Swift 中 import 的语法、模块查找机制、底层实现原理以及与 Objective-C 的互操作。
...
| 文本接口格式 | 不支持 | `.swiftinterface` ([Library Evolution](./Swift二进制兼容性.md#library-evolution)) |
| 懒加载 | ✅ 支持 | ✅ 支持 |
| 自动链接 | ✅ 支持 | ✅ 支持 |
| 子模块 | ✅ 支持 | ✅ 支持 |
...
3. **稳定的文本接口**：`.swiftinterface` 文件提供跨编译器版本的稳定 [ABI](./Swift二进制兼容性.md)
...
这是 Swift 相比 Clang Module 的重要创新。Clang 的 `.pcm` 文件是二进制格式，依赖编译器版本，而 Swift 提供了 `.swiftinterface` 文本格式，用于支持 **[Library Evolution](./Swift二进制兼容性.md#library-evolution)**（库演进）。
...
关于 Clang 源码中模块自引用限制的详细实现，请参考：[Objective-C中import详解](./Objective-C中import详解.md#模块自引用限制为什么同一-pod-或-framework-内部不能用-import)
