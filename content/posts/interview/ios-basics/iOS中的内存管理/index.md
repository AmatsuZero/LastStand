...
> *非逃逸（non-escaping）指闭包或值的生命周期不超出当前函数作用域。若值被存储到堆上的属性或逃逸闭包中捕获，则会分配在堆上。详见[值类型和引用类型的区别]({{< relref "/posts/interview/ios-basics/值类型和引用类型的区别" >}}#值类型的逃逸)。
...
苹果引入了非指针isa（Non-pointer isa）优化，利用isa指针中的空闲位来存储引用计数。关于isa的详细介绍，请参考[Objective-C底层原理-NSObject]({{< relref "/posts/interview/ios-basics/Objective-C底层原理-NSObject" >}}#优化的isa指针non-pointer-isa)。
...
侧表不仅存储大引用计数，还管理弱引用关系。当对象释放时，运行时会自动将所有指向该对象的弱引用置为nil。关于弱引用的底层实现原理，请参考[weak详解]({{< relref "/posts/interview/ios-basics/weak详解" >}})。

关于Objective-C对象的完整内存布局，请参考[Objective-C底层原理-NSObject]({{< relref "/posts/interview/ios-basics/Objective-C底层原理-NSObject" >}}#内存分布)。
...
关于Swift类的内存布局，请参考[Swift底层原理-结构体、类和协议]({{< relref "/posts/interview/ios-basics/Swift底层原理-结构体-类和协议" >}}#纯swift类的内存布局)。
...
关于存在容器的完整内存布局和协议方法调用流程，详见[Swift底层原理-结构体、类和协议]({{< relref "/posts/interview/ios-basics/Swift底层原理-结构体-类和协议" >}}#存在容器existential-container)。
...
关于RunLoop的详细介绍，请参考[RunLoop]({{< relref "/posts/interview/ios-basics/runloop" >}}#autorelease-pool-与-runloop)。
...
- **内存映射文件**：通过[mmap]({{< relref "/posts/interview/ios-basics/mmap详解" >}})映射的文件内容
...
2. **利用Clean Memory特性**
   - 使用[mmap]({{< relref "/posts/interview/ios-basics/mmap详解" >}})映射大文件而非全部读入内存
   - 避免修改通过mmap映射的只读数据
