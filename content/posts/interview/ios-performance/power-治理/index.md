...
> 耗电治理是 APM 资源子系的一部分，与稳定性、流畅性、启动指标共用同一套监控与告警基础设施。完整的 APM 建设方案见 [APM 系列]({{< relref "/posts/interview/ios-apm/APM" >}})：[数据采集]({{< relref "/posts/interview/ios-apm/数据采集" >}})、[B端平台设计]({{< relref "/posts/interview/ios-apm/B端平台设计" >}})。
