...
本文聚焦 JetsamEvent 日志本身：存放位置、文件结构、字段含义、`reason` 类型、案例解读、自动化批量解析脚本，以及机型内存上限/iOS 版本差异等实战要点。OOM 监控治理的整体方法论见 [崩溃-治理]({{< relref "/posts/interview/ios-performance/crash-治理" >}}#8-oomout-of-memory崩溃)。
