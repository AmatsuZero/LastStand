...
> 崩溃治理是 APM 稳定性子系的核心模块。完整的监控建设、多类型稳定性问题（Watchdog/OOM/CPU/IO）的统一治理与 Zombie/Coredump/MemoryGraph 等高级归因能力，请参考 [APM 系列](../APM/APM.md)：[指标体系](../APM/APM-指标体系.md)、[业界方案](../APM/APM-业界方案.md)。
...
> JetsamEvent 日志不含线程堆栈，但完整记录了强杀那一刻的整机内存画像，是 OOM 归因、阈值反推与后台生命周期治理最权威的一手数据。字段逐项详解、`reason` 深度解读、自动化批量解析脚本与机型阈值矩阵见 [JetsamEvent 日志解读](./JetsamEvent日志解读.md)。
...
> 本节聚焦"拿到崩溃后怎么做"，字段层面的字典式解读（`bug_type` / `exception` / `termination` / 寄存器 / 魔法码等）见 [崩溃日志解读](./崩溃日志解读.md)。
