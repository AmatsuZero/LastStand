...
不过转换过程中，我们还是遇到了一个关键难题：视频转骨骼动画是逐帧输出的，30FPS 的 3 秒动作就有 90 个关键帧。出现穿模时，不能只改某一帧，否则会造成前后帧突变，播放时会出现抖动。

为了解决这个问题，我们在 Blender 侧搭了一套 **MCP 工具链**，把“看、想、做、验”拆成可调用能力。

#### 工具链搭建（MCP + Blender）

我们把修复流程拆成「检测 → 分段 → 联动修复 → 复验」四段，每一段都由 MCP 直接调用 Blender 能力完成：

1. **穿模检测能力**：输出穿模骨骼、帧区间、碰撞严重度，先明确"哪里坏了"；
2. **多视角截图能力**：支持三视图与多帧拼接，让大模型直观看到姿态变化；
3. **曲线理解能力**：读取 F-Curve 变化趋势，自动识别预备、引拍、击球、收拍等动作阶段；

> **什么是 F-Curve？**
> 
> F-Curve（Function Curve，函数曲线）是 Blender 等 3D 动画软件中用于描述动画属性随时间变化的核心数据结构。在骨骼动画中，每个骨骼的每个属性（如位置 X/Y/Z、旋转 W/X/Y/Z、缩放等）都会对应一条 F-Curve。
> 
> **F-Curve 的作用：**
> - **可视化动画数据**：将关键帧之间的过渡以曲线形式展现，动画师可以直观看到动作的速度变化（曲线斜率）、加速度等
> - **精确控制动画节奏**：通过编辑曲线上的关键点和切线，可以调整动作的缓入（Ease In）、缓出（Ease Out）效果
> - **识别动作阶段**：通过分析 F-Curve 的变化趋势，可以自动识别动作的不同阶段（如网球的引拍、击球、收拍）
> 
> **在我们的项目中**：通过读取骨骼旋转的 F-Curve，MCP 工具可以识别出"预备阶段曲线平缓、引拍阶段曲线上升、击球瞬间曲线剧烈变化、收拍阶段曲线回归平稳"这样的特征，从而自动将动作分段处理。4. **关键帧编辑能力**：支持关键帧精简、批量偏移、多骨骼联动调整（如 RightArm / RightForeArm 协同修复）；
5. **快照回滚能力**：每轮调整前保存快照，试错后可秒级回退，快速逼近最佳偏移量。

#### 三轮修复策略（从粗到细）

1. **第一轮：粗修穿模**。先处理明显的肘/前臂/球拍穿插，优先把“看得见的错误”压下去；
2. **第二轮：分段细修**。按动作阶段（预备、引拍、击球、收拍）做小幅联动偏移，避免局部修好、整体变形；
3. **第三轮：曲线平滑**。统一关键帧过渡和切线，消除帧间突变，减少抖动感。

#### 自动复验与回滚

每轮修复后都会自动复验：是否仍有穿模帧、关键关节曲线是否连续、击球关键阶段节奏是否被破坏。若不满足阈值，直接回滚到上一个快照继续调整。

这套 MCP 工具链跑通后，修复不再是“人工逐帧硬改”，而是形成了可重复执行的闭环流程。

![动作绑定错误导致的穿模发球示范](wrong-serve-demo.gif)

![Blender中进行关键帧联动微调](blender-motion-binding.gif)

完整流程如下：

![动作资产穿模修复流程](motion-asset-workflow.webp)

#### 修复结果（GIF）

发球动作：

![发球动作修复结果](fixed-serve.gif)

反手击球：

![反手击球修复结果](fixed-backhand-hit.gif)

反手小球：

![反手小球修复结果](fixed-backhand-drop.gif)

正手小球：

![正手小球修复结果](fixed-forehand-drop.gif)
