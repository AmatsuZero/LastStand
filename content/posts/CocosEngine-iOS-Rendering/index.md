...
### Q1：CADisplayLink 和 CVDisplayLink 的区别是什么？Cocos 如何选择？
...
| 平台 | 默认驱动 | 备选驱动 | 切换条件 |
|------|---------|---------|---------|
| iOS | `CADisplayLink` | `dispatch_source` timer | `CocosTimerType` 枚举切换 |
| macOS | `CVDisplayLink`（`framerate=0`） | `NSTimer`（`framerate>0`） | `framerate` 参数动态切换 |
...
**两类缓存体系**：
- **状态缓存**（如 `_cullMode`、`_pipelineState`）：单值比对，O(1)
...
| 屏幕空间 Y | 向上（`screenSpaceSignY = -1.0f`） | 向下（`screenSpaceSignY = 1.0f`） |
| 裁剪空间 Y | 向上（`clipSpaceSignY = 1.0f`） | 向下（`clipSpaceSignY = -1.0f`） |

这些差异在 `CCMTLDevice` 构造函数中通过 `_caps` 配置，直接影响**投影矩阵**的构造——同样的场景数据在不同 API 下需要不同的投影矩阵才能获得一致的渲染结果。这也是跨平台引擎必须在设备初始化阶段就明确平台特征的根本原因。
